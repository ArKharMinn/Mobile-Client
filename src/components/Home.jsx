...
const Home = () => {
  ...
  const getCategoryData = async (id) => {
    ...
    try {
      if (id) {
        const response = await api.post("/getByCategory", { id: id });
        setPosts(response.data.data);
      } else {
        await getProducts();
      }
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="w-1/3 relative">
              ...
              <button
                ...
                className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 text-gray-500 hover:text-blue-600 focus:outline-none"
              >
                <svg
                  className="h-5 w-5"
                  fill="none"
